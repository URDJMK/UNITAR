...
`video/` holds an MP4 of each scene, recorded from the real page — 1920 × 1080, H.264, no audio, under 1 MB each:

| File | Length | Shows |
|---|---|---|
| `video/01-pick-the-scene.mp4` | 7.3 s | the AI pick appearing, the cut to the close-up, the shot locking |
| `video/02-scan-and-erase.mp4` | 7.2 s | scan → matte → background erased → actor on a new plate |
| `video/03-one-prompt-full-cut.mp4` | 11.4 s | prompt → clips assembling → grade and subtitles → playback → the time-saved figures |

Insert one with **Insert → Video → Video from File**, then in the **Playback** tab set **Start: Automatically** (and **Loop until Stopped** if you want it to run while you talk over it). They play from inside the `.pptx` with no browser, no server and no internet — use these when you can't rehearse on the presenting machine, and the live HTML when you can.
